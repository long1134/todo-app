      ...
      render={({ field, fieldState }) => (
        <Box sx={containerSx}>
         <Box sx={{display:"flex",justifyContent:"space-between",fontWeight:500}}>
              <InputLabel htmlFor={id ?? name} style={{ fontWeight: "normal",color:"black" }} error={fieldState.error != null}>
                {label}
              </InputLabel>
            {isPassword  && <Link  to="/auth/login"><Typography sx={{textDecoration:"underline",color:"#68ca9a"}}>Forgot Password ?</Typography></Link>}
         </Box>
          <OutlinedInput
            id={id ?? name}
            {...other}
            {...field}
            sx={{ width: "100%", mt: 1,backgroundColor:"#f5f5f6",height:"60px",fontWeight:500,pl:1,
